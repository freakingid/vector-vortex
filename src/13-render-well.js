...
function wellBandColor(level, rng) {
  ...
  // ⛔ An indexed loop, never `for…of` (GDD 17's budget; CS017 P1, S3
  // addendum): this runs several times a frame, and a `for…of` takes an array
  // iterator on every call that the engine does not always optimise away.
  const bands = C.BAND_COLORS;
