...
// ---------------------------------------------------------------------------
// Debug well cycler — P3's own preview loop.
// ---------------------------------------------------------------------------
// 02-state.js and 23-main.js (the real game loop and state machine) are still
// placeholders this phase; nothing later than this module exists to drive a
// canvas yet. This bootstrap exists solely so "a debug key cycles all sixteen
// wells" is true of the shipped build today, without inventing game state or
// a loop shape a later phase hasn't decided. It owns nothing but a well index
// and a level counter (for band colour), both local to this block, and steps
// entirely out of the way — no globals beyond the debug key listener itself —
// once 23-main.js exists and starts driving its own render calls.
//
// ⛔ Guarded on `window.document` so the headless test harness (which stubs
// addEventListener as a no-op but still calls this file's top level) never
// throws for lack of a real canvas element.
if (typeof window !== "undefined" && typeof document !== "undefined") {
  (function () {
    let debugWellIndex = 0;
    let debugLevel = 1;
    ...
    function renderDebugFrame() {
      ...
      drawWell(ctx, WELLS[debugWellIndex], debugLevel, null, 0);
    }

    document.addEventListener("keydown", (e) => {
      if (e.key !== "w" && e.key !== "W") return;
      debugWellIndex = (debugWellIndex + 1) % WELLS.length;
      renderDebugFrame();
    });
