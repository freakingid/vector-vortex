...
- ✗ **Not met by Classic play — filter sweep audible end to end.** The sweep maps the full 600 Hz → 18 kHz range (`test-cs010-p2.js`), but Classic has no combo input, its weights sum to 0.85, and the played maximum is **0.668** (plan §1.1; the eighth soak measured the same 0.668), so the sweep peaks near 5.8 kHz and never fully opens. Nothing was rescaled (Paul's D6). CS012 re-measures Overdrive with its combo.
