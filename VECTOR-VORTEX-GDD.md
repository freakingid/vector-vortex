...
├── tools/                       # design instruments — music-lab, sfx-lab, well-lab, glow-lab, feel-lab, reach-probe (Node)
...
- ◐ **All 16 wells render and play.** Render ✅: `screenPos` finite at every lane and sampled depth of every well (`test-cs001-p2.js:109`), and `drawWell()` and the real `draw()` throw nothing on all sixteen (`test-cs001-p3.js:63`, `test-cs002-p2.js:393`). Play is carried per mechanic on all sixteen — the Mimic and its shot stepped on every well (`test-cs013-p4.js:704`), the well cycle stranding nothing (`test-cs003-p2.js`) — and soaked on the six open wells (`test-cs005-p5.js:815`). ⚠ **No played session is asserted to have visited every well.**
