...
⛔ **Shipped, CS012 P4 — and it is applied AT THE KILL SITES, never inside `addScore()`** (`PLANNED-FEATURES-CS012.md` O4, R6). Each of the four kill lines in `09-collision.js` reads `addScore(e.points() * comboMult())`: a kill is scored at the multiplier in force and then raises it. `addScore()` is unchanged and stays the one writer and the one life-awarder.
- ⛔ **What is multiplied is exactly what builds it** (O4): kill points at the three kill sites — a shot, the rim sweep, and **both** Purge uses — and every kill there builds the combo. That is one rule, and it is the kill sites' existing one (`points()` on the false → true `dead` edge).
