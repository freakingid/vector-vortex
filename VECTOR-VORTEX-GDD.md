...
| `pulse` mix, gated / sample peak | −24.4 dB / 0.303 | −32.0 dB / 0.421 | −33.6 dB / 0.347 |
...
In the struck column, the balance between layers held within 0.5 dB of P2. ✅ **The shipped balance is Paul's, by ear in the lab (2026-09-16).** He raised `title`'s ground, and `pulse`'s bassline, heart and tick, to the slider's 0.300 top, and marked every layer PASS. `title` ports exactly as he set it (headroom 0.4404). `pulse` at his gains reads 0.7771 against the tone's 0.45, so all six of its gains are trimmed together by 5.3 dB (headroom 0.4196). His balance holds, and the level drops. ⚠ **So the game's `pulse` is 5.3 dB quieter than he heard it**, and buying that back is CS010's mix call. ⚠ **The mix is 5–8 dB quieter at unity and peaks higher**, because struck notes crest together on the beat. Two limits set the gains: §11.8's headroom gate, and a sample peak of 0.42 for both mixes. ⚠ **A rendered sample peak and §11.8's envelope sum measure different things, and neither bounds the other.** A detuned layer is two oscillators, so it can crest at twice its envelope. The P2 `title` mix already peaked at 0.319 over its 0.3005 sum.
...
⛔ **The headroom gate (`scratchpad/test-cs009-p5.js`)** runs at the default volumes. The tone's peak at `master` (recipe gain × SFX bus) must be at least the loudest overlap of note peaks at `master` over a whole loop. It must hold for `pulse` and for `title`. MEASURED at P5: 0.450 against 0.434 on `pulse` and 0.3005 on `title`. Re-measured after the articulation pass (§11.3): 0.4201 on `pulse` and 0.3424 on `title`. Re-measured again at Paul's 120 BPM and balance: 0.4196 on `pulse` (after a 5.3 dB trim; 0.7771 without it) and 0.4404 on `title`. ⚠ The 0 dB ratio is provisional. The hardware check is a skipped playtest (`SKIPPED-PLAYTESTS.md`). ⚠ **"At every intensity tier" is untested until tiers exist.** CS009's tracks are untiered, which is the loudest a track can be, and CS010's sweep and layers re-run the gate.
