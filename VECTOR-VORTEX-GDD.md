...
- **The ending** is `quitToTitle()`, on any of: a rotate or a held button in the sampled struct, ANY named action (which then does nothing else — no pause, bench, telemetry toggle, export or save), the demo's game over (at once: no game-over screen), and `ATTRACT_LENGTH` 45 s ⚠ of simulation time. ⛔ An ending inside a step keeps the step's struct, so the title's entry step latches what is held and the ending press confirms no row. ⚠ An UNBOUND key reaches neither the struct nor a named action and does not end it, though the line says ANY KEY (`STATUS.md`).
- **What it shows:** the board and the HUD as any run (§10.4's H4), and `ATTRACT_LINE` `DEMO — PRESS ANY KEY` ⚠ in the prompt band through `drawText()`, never a prompt row. Music is the mode's track through the unchanged `musicStateFor()`; ⚠ on a fresh page load the demo is silent until a gesture, which ends it.
