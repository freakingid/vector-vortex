...
- ⛔ **No allocating expression on the DRAW path** (S3): `projectPoly()`, `drawShot()`'s pair, `drawText()`'s font string and `drawPrompt()`'s fade colour are cached, each asserted by an observable its old line fails. The step path's `.filter()` is §16.1's invariant and stands. ⚠ **Not yet met whole — found by P1, not among S3's four, and Paul's call** (`STATUS.md`): the HUD builds its score, level and combo strings on every play frame (`hudScoreText` / `hudLevelText` / `hudComboText`, ~70 B), `wellBandColor()` iterates `C.BAND_COLORS` with `for…of` (~40 B), and the game-over screen rebuilds its three lines on every frame (`Game.draw()`). What the ENGINE still allocates is data, not a rule — MEASURED by `tools/perf-probe.js` (headless Chromium 153, software raster; the build `ed8474d` + P1's four fixes, which P1 commits), bytes per `draw()` after 300 draws / at steady state (+5,000): **budget board 6,184 / 5,059** (8,200 / 6,929 before the fixes), **empty board 1,381 / 213** (3,017 / 1,856). The budget board's residue is V8's, chiefly boxed doubles under `entityPoints()`, whose body allocates nothing (plan S3-B, not chased).
