...
The scheduler emits kick and snare events to the render layer; the rim pulses on the kick (⚠ as shipped in CS010 P4: one `beat` event, on `pulse`'s `heart`, below). ⛔ **Driven by the scheduler, not an `AnalyserNode`** — an analyser adds latency, and tightness is the point.
