...
**Shipped, CS004 P2 — one of the three rows.** `CARGO` (`07-enemies.js`) maps a cargo name to what it splits into, and ⛔ only `vaulter` is buildable. ⚠ **CS005 P2 and P3 landed the Drifter and the Surger as ENEMY_KINDS rows and deliberately did NOT add their `CARGO` rows** — the cargo rows and their `carrierDrifter` / `carrierSurger` variants are P4's, one phase per thing, so each entity is judgeable on the bench before it is judgeable inside a hull. The table is a shape with one entry in it and is **not a weighted draw yet** — §8's "cargo weights shift toward Drifter/Surger" is heat, and heat is CS006's.
