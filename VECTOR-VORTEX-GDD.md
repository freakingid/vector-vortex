...
- ⛔ **No allocating expression on the DRAW path** (S3): `projectPoly()`, `drawShot()`'s pair, `drawText()`'s font string and `drawPrompt()`'s fade colour are cached, and `wellBandColor()` walks `C.BAND_COLORS` by index, never `for…of` (S3 addendum, Paul), each asserted by an observable its old line fails. The step path's `.filter()` is §16.1's invariant and stands. ⚠ **Not yet met whole — found by P1 and still Paul's call** (`STATUS.md`): the HUD builds its score, level and combo strings on every play frame (`hudScoreText` / `hudLevelText` / `hudComboText`, ~70 B), and the game-over screen rebuilds its three lines on every frame (`Game.draw()`). What the ENGINE still allocates is data, not a rule — MEASURED by `tools/perf-probe.js` (headless Chromium 153, software raster; the build with P1's five fixes), bytes per `draw()` after 300 draws / at steady state (+5,000): **budget board 6,055 / 5,014** (8,200 / 6,929 at `ed8474d`), **empty board 1,247 / 170** (3,017 / 1,856). The budget board's residue is V8's, chiefly boxed doubles under `entityPoints()`, whose body allocates nothing (plan S3-B, not chased).
